...
TEST(Robot_IO, streaming_encode_decode) {
  ...
  auto encoded = robot_io::Streaming::encode(test_data);
  ASSERT_EQ(encoded.size(), test_data.size() + robot_io::Streaming::SIZE_FIELD_LENGHT);

  auto size = encoded.front();
  ASSERT_EQ(size, (uint8_t)test_data.size());

  std::vector<uint8_t> decoded;
  ASSERT_TRUE(robot_io::Streaming::decode(encoded, decoded));
...
TEST(Robot_IO, streaming_decoded_payload_removal) {
  ...
  std::vector<uint8_t> decoded;
  ASSERT_TRUE(robot_io::Streaming::decode(encoded_test_data, decoded));

  check_decoded_data(decoded, data1, size1);

  // check that the encoded data vector only contains the second packet
  ASSERT_TRUE(encoded_test_data.size() == size2 + robot_io::Streaming::SIZE_FIELD_LENGHT);
  std::vector<uint8_t> decoded2;
  // print_container(encoded_test_data);
  ASSERT_TRUE(robot_io::Streaming::decode(encoded_test_data, decoded2));
...
TEST(Robot_IO, streaming_decode_incomplete_packet_fail) {
  ...
  std::vector<uint8_t> decoded;
  ASSERT_FALSE(robot_io::Streaming::decode(encoded_test_data, decoded));

  // check that no data have been removed from the encoded vector
  ASSERT_TRUE(encoded_test_data.size() == data.size() + robot_io::Streaming::SIZE_FIELD_LENGHT);
}